...
<head>
    ...
    <title>Work Experiences - Regret Consultancy</title>
...
<body class="font-sans">
    ...
    <main class="pt-16 p-4 md:p-6 md:ml-64 mt-10">
        <div class="rounded-xl shadow-sm border border-slate-200 overflow-hidden">
            <div
                ...
                <div>
                    <h3 class="text-xl font-bold text-[#1e293b]">Work Experiences</h3>
                    <p class="text-sm text-slate-500 mt-1">Manage your work experiences</p>
                </div>
                <a href="{{ route('workexperiences.create') }}"
                    ...
                    <table class="w-full">
                        <thead class="bg-slate-50">
                            <tr>
                                ...
                                <th class="px-6 py-4 text-left text-sm font-semibold text-slate-600">
                                    <a href="javascript:void(0)" onclick="sortBy('years')"
                                        class="flex items-center gap-1 hover:text-[#0257b3]">
                                        Years
                                    </a>
                                </th>
                                    ...
                                    <td class="px-6 py-4">
                                        @if($workExperience->image)
                                            <img src="{{ asset('uploads/work-experiences/' . $workExperience->image) }}"
                                                alt="{{ $workExperience->title }}" class="w-16 h-16 object-cover rounded-lg">
                                        @else
                                            <span class="text-slate-400">No Image</span>
                                        @endif
                                    </td>
                                    <td class="px-6 py-4 text-sm text-slate-800 font-medium">{{ $workExperience->title }}</td>
                                    <td class="px-6 py-4 text-sm text-slate-700">{{ $workExperience->years }}</td>
                                    <td class="px-6 py-4">
                                        <span
                                            class="px-3 py-1 rounded-full text-xs font-semibold {{ $workExperience->status === 'active' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                            {{ ucfirst($workExperience->status) }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4">
                                        <a href="{{ route('workexperiences.edit', $workExperience) }}"
                                            class="text-[#0257b3] hover:text-[#0d9488] font-medium text-sm mr-3">Edit</a>
                                        <button type="button" onclick="deleteWorkExperience({{ $workExperience->id }})"
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="px-6 py-8 text-center text-slate-500">
                                        No work experiences found. <a href="{{ route('workexperiences.create') }}"
                                            class="text-[#0257b3]">Add one now</a>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
    ...
    <script>
        ...
        function loadWorkExperiences() {
            let url = '{{ route("workexperiences.index") }}?' + new URLSearchParams({
        ...
        function deleteWorkExperience(workExperienceId) {
            ...
            popup.innerHTML = `
                <div class="bg-white rounded-2xl shadow-2xl p-8 flex flex-col items-center">
                    ...
                    <h3 class="text-2xl font-bold text-gray-900 mb-2">Delete Work Experience</h3>
                    ...
                    <div class="flex gap-4">
                        ...
                        <button onclick="confirmDelete(${workExperienceId})" class="px-6 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600">Delete</button>
                    </div>
                </div>
            `;
        ...
        async function confirmDelete(workExperienceId) {
            document.getElementById('deletePopup').remove();

            try {
                const response = await fetch(`/admin/workexperiences/${workExperienceId}`, {
                    method: 'DELETE',
                    headers: {
                ...
                loadWorkExperiences();
                showSuccessPopup(data.message || 'Work Experience deleted successfully!');
